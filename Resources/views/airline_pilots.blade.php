      ...
      <table class="table table-sm table-striped table-borderless mb-0 text-center">
        ...
        @foreach($pilots as $user)
          <tr>
            ...
            <td class="align-middle">
              <span class="badge {{ Dsp_UserStateBadge($user->state) }}">{{ \App\Models\Enums\UserState::label($user->state) }}</span>
            </td>
          </tr>
        @endforeach
      </table>
    </div>
    <div class="card-footer p-1 text-right">@lang('DisposableAirlines::common.totpilots'): {{ $pilots->count() }}</div>
  </div>
</div>
